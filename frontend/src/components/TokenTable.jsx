...
const getChainBadge = (chain) => { 
  const colors = { 
    ...
    optimism: 'bg-green-400/20 text-green-300', 
  }; 
  return colors[chain?.toLowerCase()] || 'bg-gray-500/20 text-gray-400'; 
}; 
      ...
      <table className="w-full text-sm"> 
        ...
        <tbody> 
          {tokens.map((token, index) => { 
            // Unique Identifier targeting token_address first, falling back to pair_address
              ...
              > 
                ...
                <td className="py-3 px-2"> 
                  <div className="flex items-center gap-2"> 
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-green-500/30 to-blue-500/30 flex items-center justify-center text-xs font-bold text-white"> 
                      {token.symbol?.[0] || '?'} 
                    </div> 
                    <div> 
