...
const Watchlist = () => {
  ...
  const loadWatchlist = async () => {
    ...
    try {
      ...
      if (data.success) {
        const filtered = data.data.filter(t => watchlist.includes(t.pair_address));
        setTokens(filtered);
      }
    } catch (error) {
        ...
        <div className="flex-1 p-4">
          ...
          {watchlist.length === 0 ? (
            <div className="text-center py-10 text-gray-400">
              <p className="text-lg">📭 No tokens in watchlist</p>
              <p className="text-sm mt-1">Go to Home and add tokens to your watchlist</p>
            </div>
          ) : loading ? (
            <div className="text-center py-10 text-gray-400">Loading watchlist...</div>
          ) : (
